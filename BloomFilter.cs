...
///     32, 16, 8, 4, 2
///     In the first step we extract the bright spots from the original image. If not specified otherwise this happens in
///     full resolution.
///     We can do that based on the average RGB value or Luminance and check whether this value is higher than our
///     Threshold.
...
///     Afterward we upsample again, but blur in this step, too.
...
public class BloomFilter : Configurable, IDisposable {
	...
	///     The intended format for the rendertargets. For normal, non-hdr, applications color or
	///     rgba1010102 are fine NOTE: For OpenGL, SurfaceFormat.Color is recommended for non-HDR applications.
	/// </param>
	...
	public void Load(GraphicsDevice graphicsDevice, ContentManager content,
		...
		SetBloomPreset(BloomPresets.Custom);
	}

	protected override void LoadSettings() {
		UpdateResolution(Settings.ResolutionX, Settings.ResolutionY);
	}
	...
	private void SetBloomPreset(BloomPresets preset) {
		switch (preset) {
			...
			case BloomPresets.Custom: {
				_bloomStrength1 = 0.9f;
				_bloomStrength2 = 1;
				_bloomStrength3 = 1;
				_bloomStrength4 = 2.5f;
				_bloomStrength5 = 5;
				_bloomRadius5 = 2.5f;
				_bloomRadius4 = 2.0f;
				_bloomRadius3 = 2.0f;
				...
				BloomThreshold = 0.125f;
				BloomStrengthMultiplier = 1f;
				
				BloomStreakLength = 1.1f;
				BloomDownsamplePasses = 5;
				break;
			}
		}
